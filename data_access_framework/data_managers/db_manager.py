    ...
    def search(self, criteria: Dict[str, Any]) -> List:
        ...
        try:
            ...
            for key, value in criteria.items():
                if key in ['id', 'title', 'name', 'email', 'username', 'studio_id', 'player_id', 'game_id']:
                    where_parts.append(f"{key} LIKE ?")
                    params.append(f"%{value}%")
                elif key in ['available', 'active', 'completed', 'multiplayer']:
                    where_parts.append(f"{key} = ?")
                    params.append(1 if value else 0)
                elif key in ['purchase_date', 'last_played', 'created_at', 'updated_at']:
